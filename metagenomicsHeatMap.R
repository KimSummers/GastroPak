# metagenomicsHeatmap
#
# Heatmap Metagenomics data for GastroPak - code adapted from code supplied by Emma Travis
#
# file metagenomicsHeatmap
#
# inputs
...
# 1.00       J K Summers  11/09/24  Wellington Lab - School of Life Sciences - University of Warwick

metagenomicsHeatmap <- function(dataFile, metaDataFile, sampleNumber, 
                                riversFile, plotsDir) {
  ...
  mgData <- read.csv(dataFile, sep = "\t")
  
  mgData <- mgData[, c(1, which(substr(colnames(mgData), 1, 3) == "PAK"))]
  
  # transform rmData into long format 
  mgDataLong <- pivot_longer(mgData, cols = starts_with("PAK"), names_to = "Sample_ID",
                             values_to = "relative_abundance")
  
  #statistical analysis
  # test for normality
  ggdensity(mgDataLong$relative_abundance, fill = "lightgray")
  ggqqplot(mgDataLong$relative_abundance)
  shapiro.test(mgDataLong$relative_abundance)
  
  # Shapiro-Wilk normality test
  # data:  mgDataLong$relative_abundance
  
  # use log10 and NA instead of 0 values and test again for normality
  mgDataLongLog <- mutate(mgDataLong, Log_RA = log10(mgDataLong$relative_abundance))
  
  ggdensity(mgDataLongLog$Log_RA, fill = "lightgray")
  ggqqplot(mgDataLongLog$Log_RA)
  shapiro.test(mgDataLongLog$Log_RA)
