...
GPMetagenomicsNMDSGenusSeasonal <- function(metagenomicDataFile, metaDataFile,
                                            riversFile, phylaFile, distMatrixFile,
                                            categories, sampleOrder, campaignType,
                                            plotsDir) {
  ...
  phylas$Kingdom <- sub("k__", "", phylas$Kingdom)
  phylas$Phyla <- sub("p__", "", phylas$Phyla)
  phylas$Class <- sub("c__", "", phylas$Class)
  phylas$Order <- sub("o__", "", phylas$Order)
  phylas$Family <- sub("f__", "", phylas$Family)
  phylas$Genus <- sub("g__", "", phylas$Genus)
  phylas$species <- sub("s__", "", phylas$species)
  ...
  mgGenusWide <- mgGenusWide[rownames(mgGenusWide) %in% metaDataWide$Barcode, ]
  ...
  nmds.plot.genus <- nmds.plot.genus + labs(color = "Location", shape = "Location")
  # add legend labels for Location and Month
  nmds.plot.genus <- nmds.plot.genus + ggtitle("Species NMDS - Grouping by river")
  ...
  ggsave(paste(plotsDir, season, " NMDS Genus All by river main.pdf", sep = ""), nmds.plot.genus)
  ...
  ggsave(paste(plotsDir, season, " NMDS Genus All by river main plus vectors.pdf", sep = ""), nmds.plot.genus3)
  ...
  ggsave(paste(plotsDir, season, " NMDS Genus All by river main plus top 20 vectors.pdf", sep = ""), nmds.plot.genus4)
  ...
  ggsave(paste(plotsDir, season, " NMDS Genus All by river main plus classes.pdf", sep = ""), nmds.plot.genus5)
